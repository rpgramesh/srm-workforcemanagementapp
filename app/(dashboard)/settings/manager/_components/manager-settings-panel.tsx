...
export function ManagerSettingsPanel({ actorRole, myStaffCount, totalStaffCount, departmentCounts }: ManagerSettingsPanelProps) {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div>
              <Field id="deptSwitch" label="Department">
                <Select id="deptSwitch" value={department} onChange={(e) => setDepartment(e.target.value)}>
                  {DEPARTMENTS.map((d) => (
        ...
        <CardContent className="space-y-5">
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <div className="text-xs uppercase tracking-wider text-slate-400">My Direct Reports</div>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <div className="text-xs uppercase tracking-wider text-slate-400">{department} Headcount</div>
              <div className="mt-2 text-3xl font-bold text-white">{departmentCounts[department] ?? 0}</div>
              <div className="mt-2 text-xs text-slate-500">of {totalStaffCount} total</div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <div className="text-xs uppercase tracking-wider text-slate-400">Currently On Duty</div>
                ...
                return (
                  <div key={s.id} className="flex items-center gap-4 px-5 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="truncate text-sm font-medium text-white">{s.firstName} {s.lastName}</div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Floor Operations</CardTitle>
        ...
        <CardContent className="space-y-5">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Field id="openStart" label="Open Hours Start">
              <Input id="openStart" type="time" value={openHoursStart} onChange={(e) => setOpenHoursStart(e.target.value)} />
            </Field>
            <Field id="openEnd" label="Open Hours End">
              <Input id="openEnd" type="time" value={openHoursEnd} onChange={(e) => setOpenHoursEnd(e.target.value)} />
            </Field>
      ...
      <Card>
        ...
        <CardContent className="space-y-5">
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
              ...
              <div className="mt-2 flex items-baseline gap-2">
                <div className="text-3xl font-bold text-emerald-300">28.4%</div>
                <span className="text-xs text-slate-500">target 30%</span>
              </div>
              <div className="mt-3 h-2 w-full overflow-hidden rounded-full bg-white/5">
                <div className="h-full w-[28.4%] rounded-full bg-gradient-to-r from-emerald-400 to-teal-400" />
              </div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
              <div className="text-xs uppercase tracking-wider text-slate-400">Weekly Payroll</div>
              <div className="mt-2 text-3xl font-bold text-white">$24,180</div>
              <div className="mt-2 text-xs text-slate-500">38 scheduled this week</div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
              <div className="text-xs uppercase tracking-wider text-slate-400">Pending Swaps</div>
              <div className="mt-2 text-3xl font-bold text-amber-300">5</div>
              <div className="mt-2 text-xs text-slate-500">Awaiting manager approval</div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Button variant="subtle" onClick={handleDownloadPayroll}>
              ↓ Payroll CSV
            </Button>
            <Button variant="subtle" onClick={handleDownloadLabor}>
              ↓ Labor % CSV
            </Button>
            <Button variant="subtle" onClick={handleDownloadHours}>
              ↓ Hours Summary
            </Button>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Staff Scheduling Controls</CardTitle>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Departmental Resources</CardTitle>
        ...
        <CardContent className="space-y-5">
          <Field id="resourceDept" label="Department">
            <Select id="resourceDept" value={selectedDeptForResources} onChange={(e) => setSelectedDeptForResources(e.target.value)}>
              {DEPARTMENTS.map((d) => (
                <option key={d} value={d}>{d}</option>
              ))}
            </Select>
          ...
          <div>
            <div className="mb-2 text-xs font-medium uppercase tracking-wider text-slate-400">On-Duty Badges</div>
